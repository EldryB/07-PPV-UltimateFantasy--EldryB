...
class BattleState(BaseState):
    ...
    def update(self, dt: float) -> None:
        if not self.battle_started:
            ...
            self.turn_queue = list(self.party.characters.values()) + self.enemies
            self.turn_index = 0
            
            for entity in self.turn_queue:
                entity.rest_turns = 0
        ...
        for _ in range(len(self.turn_queue)):
            entity = self.turn_queue[self.turn_index]
            self.turn_index = (self.turn_index + 1) % len(self.turn_queue)
            if not entity.dead:
                found_alive = True
                break
        ...
        if entity.rest_turns > 0:
            turns_left = entity.rest_turns
            entity.rest_turns -= 1
            
            turn_str = "turn" if turns_left == 1 else "turns"
            message = f"{entity.name} is resting! {turns_left} {turn_str} to return."
    ...
    def _trigger_turn(self, entity: Any) -> None:
        if hasattr(entity, 'hpiv'): # It's a Character
            from src.states.game.BattleMenuState import BattleMenuState
            self.state_machine.push(
                BattleMenuState(self.state_machine),
                battle_state=self,
                entity=entity
            )
        else: # It's an Enemy
